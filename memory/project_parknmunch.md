...
**How to verify a receipt fix like this in the future**: don't just re-read the Kotlin — port the exact parsing logic (regexes, `row()`'s truncation math, `formatItem()`) to a small Node script and run real strings through it. Building a visual mockup Artifact styled like actual thermal paper (monospace, 48-char grid) from that real output is far more convincing/useful than describing the fix in prose.
