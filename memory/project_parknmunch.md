...
## Order metadata truncation fix (2026-07-03)
`create-checkout-session.js` packs the cart into a Stripe metadata field capped at 490 chars for `stripe-webhook.js` to reconstruct `order_items`. The original code did a naive `.slice(0, 487) + '...'` on the JSON string — for a large/detailed cart (stress-tested at 8 items including the new Box Builder products) this cuts mid-object, produces invalid JSON, and `safeParseOrderItems` in the webhook silently returns `[]`: the order still gets created with the correct total, but with **zero visible items** on both the admin Orders page and the printed receipt. Fixed with `packOrderMetadata()`, which packs items one at a time and stops before exceeding budget (always valid JSON), appending a `"+N more items - see Stripe session for full order"` marker (with reserved budget so the marker itself can never get squeezed out) when the cart doesn't fully fit. Only ever affects unusually large/detailed orders — small normal orders are untouched.
